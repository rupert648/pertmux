...
- **client.rs**: TUI client. Connects to daemon (fails with error screen if not running), owns `ClientState` with all UI state (selections, popup, notification). Event loop with `tokio::select!` on keyboard + daemon messages. Local navigation (j/k/Tab) with no round-trip. Tab cycles `MergeRequests → Worktrees → Projects(focus) → MergeRequests`; while Projects is focused, `j/k` scroll, `h/l` move the cursor column, `Space` applies sort (toggles direction if already sorted by that column), `Enter` selects the project. Focus-mode key handling is in `handle_project_focus_key` (returns `KeyOutcome::Handled` or `Fallthrough` so q/Esc/Tab still reach the global dispatcher). Project switching via fuzzy finder (`f` key). Also provides `stop()`, `status()`, and `cleanup()` commands. On reconnect, if `pending_changes` is non-empty, opens `ChangeSummary` modal; for live snapshots, shows toast notifications.
...
- **ui/components/**: Modular rendering components — `list_panel` (left panel with MR list or agent panes), `detail_panel` (right panel with MR detail or session info), `mr_sections` (MR and worktree block layouts), `cards` (individual MR/worktree cards), `overview` (Projects table with sortable columns: Name/MRs/OC/Busy/Idle; supports focus mode), `pipeline` (CI/CD dot visualization), `popup` (worktree actions, fuzzy filter, MR overview, activity feed popup), `notification` (toast overlay — renders `ClientState.notification`), `change_summary` (reconnect modal showing accumulated MR changes), `activity_feed` (inline activity log in the lower-right panel — label width is dynamic based on available width, GLOW_SECS = 1800 so entries stay visible for 30 minutes).
- **project_sort.rs**: `ProjectSort` enum (Name/Mrs/Oc/Busy/Idle), the `ORDER` array driving header rendering and `next_col`/`prev_col` cycling, and the `~/.local/share/pertmux/last_sort` round-trip (`save_last_sort`/`load_last_sort`). Persistence file format: `<col>,<asc|desc>`; legacy single-token form is still read.
- **project_stats.rs**: Shared aggregation of per-project Busy/Idle/MR counts (`ProjectStats`). `build_sorted_project_stats(snapshot, col, desc)` is called by both `client.rs` (to map cursor row → canonical project index) and `ui/components/overview.rs` (to render the table) so the two paths cannot drift. Sort tie-break is name-ascending and is held in a separate binding so the `desc` flip does not reverse it.
- **worktrunk.rs**: Serde types for `wt list --format=json` output (`WtWorktree`, `WtCommit`, `WtMain`, etc.). Async functions: `fetch_worktrees()`, `create_worktree()`, `remove_worktree()`, `merge_worktree()` — the latter three take a `run_hooks: bool` (the daemon passes `!config.skip_worktrunk_hooks`, default `true`); when `false` they pass `--no-hooks` to skip worktrunk hooks. Includes `format_age()` helper and unit tests.
...
- Worktree hooks: skip via `--no-hooks` (NOT the deprecated `--no-verify`, which current worktrunk aliases to `--no-hooks` anyway). Controlled by `config.skip_worktrunk_hooks` (default `false` = run hooks).
...
- Action keybindings are configurable via `[keybindings]` in the TOML config (e.g. `mr_overview`, `merge_worktree` which now defaults to `M`, `activity_feed` which defaults to `A`, `open_worktree_with_prompt` which defaults to `w`, `worktree_search` which defaults to `W` and opens a global fuzzy search over worktrees across all projects). Navigation keys (`j`/`k`/`↑`/`↓`/`Tab`/`Enter`/`Esc`/`q`) are not configurable.
