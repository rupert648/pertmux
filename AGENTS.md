...
- Action keybindings are configurable via `[keybindings]` in the TOML config (e.g. `mr_overview`, `merge_worktree` which now defaults to `M`, `activity_feed` which defaults to `A`). Navigation keys (`j`/`k`/`↑`/`↓`/`Tab`/`Enter`/`Esc`/`q`) are not configurable.
